...
const SignupPage = () => {
  ...
  const checkUsernameUniqueness = async (username: string) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/users/check_username`, 
        { username }
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL}/api/users/create_user_profile`, payload, { withCredentials: true });
  ...
  const passwordsMismatch = passwordConfirm.length > 0 && formData.password.length > 0 && formData.password !== passwordConfirm;
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 relative">
      ...
      <div className="bg-slate-900 border border-slate-800 p-8 md:p-12 rounded-[2.5rem] shadow-2xl w-full max-w-xl animate-in fade-in zoom-in-95 duration-500">
        <header className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter">Join MangaKart</h1>
          <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-2">Create your reader profile</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="relative group">
              ...
              <input
                ...
                className="signup-input pl-12 pr-12"
                required
              />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              <input type="tel" name="phone_number" value={formData.phone_number} maxLength={10} onChange={handleChange} placeholder="Phone (Optional)" className="signup-input pl-12" />
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input type={showPassword ? 'text' : 'password'} name="password" value={formData.password} onChange={handleChange} placeholder="Password (min 8)" className="signup-input pl-12 pr-12" required />
          ...
          <div className="relative">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500" />
            <input type={showConfirmPassword ? 'text' : 'password'} value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} placeholder="Confirm Password" className={`signup-input pl-12 pr-12 ${passwordsMismatch ? 'border-rose-500/50' : ''}`} required />
          ...
          {passwordsMismatch && <p className="text-rose-500 text-[10px] font-black uppercase ml-2 tracking-widest">Passwords do not match</p>}
        ...
        <p className="text-xs text-center text-slate-500 mt-8 font-bold uppercase tracking-widest">
          Already a member? <Link href="/login" className="text-indigo-400 hover:text-indigo-300 ml-1 underline decoration-2 underline-offset-4">Sign In</Link>
        </p>
      </div>
